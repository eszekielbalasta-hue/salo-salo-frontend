...
const PAY_MAP = { GCash:"gcash", PayMaya:"paymaya", PayPal:"paypal", Cash:"cash" };
const PAY_COLOR = { GCash:"#0075C9", PayMaya:"#00A862", PayPal:"#003087", Cash:"#C9922A", Unpaid:"#6B4A18" };
...
export default function OrdersPage() {
  ...
  const updateOrder = async (id, updates) => {
    try {
      await API.put(`${ORDER_API}/${id}`, updates);
      fetchOrders();
            ...
            <div className="card" key={order._id}>
              ...
              <div style={{
                ...
                <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                  {PAY_MAP[order.paymentMethod] ? (
                    <img
                      src={PAY_ICONS[PAY_MAP[order.paymentMethod]]}
                      alt={order.paymentMethod}
                      style={{ width:28, height:28, objectFit:"contain",
                        borderRadius: order.paymentMethod === "GCash" ? 6 : 3 }}
                    />
                  ) : (
                  ...
                  <span style={{
                    ...
                    color: PAY_COLOR[order.paymentMethod] || "var(--g2)",
                    fontWeight: 600,
                  }}>
              ...
              <div style={{
                ...
                <p style={{
                  fontSize:"0.56rem", fontFamily:"Cinzel,serif",
                  letterSpacing:"0.22em", textTransform:"uppercase",
                  color:"var(--g4)", marginBottom:8,
                }}>Update Payment Status</p>
                <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                  {["Paid","Pending","Failed"].map(ps => (
                    <button
                      key={ps}
                      onClick={() => updateOrder(order._id, { paymentStatus: ps })}
                      style={{
                        padding: "5px 14px",
                        fontSize: "0.62rem",
                        fontFamily: "Cinzel,serif",
                        ...
                        border: "1px solid",
                        transition: "all 0.2s",
                        background: order.paymentStatus === ps
                          ? ps === "Paid" ? "rgba(76,175,125,0.2)"
                            : ps === "Pending" ? "rgba(240,165,0,0.2)"
                            : "rgba(212,96,96,0.2)"
                          : "transparent",
                        borderColor: ps === "Paid" ? "rgba(76,175,125,0.5)"
                          : ps === "Pending" ? "rgba(240,165,0,0.5)"
                          : "rgba(212,96,96,0.5)",
                        color: ps === "Paid" ? "var(--ok)"
                          : ps === "Pending" ? "var(--warn)"
                          : "var(--err)",
                        fontWeight: order.paymentStatus === ps ? 700 : 400,
                      }}
                    >
                      {ps === "Paid" ? "✓ Mark Paid"
                       : ps === "Pending" ? "⏳ Pending"
