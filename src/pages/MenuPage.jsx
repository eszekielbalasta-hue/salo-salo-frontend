...
const PAY_COLOR = {
  GCash:"#0075C9", PayMaya:"#00A862",
  PayPal:"#003087", Cash:"#C9922A", Unpaid:"#6B4A18"
};
...
export default function MenuPage({ token }) {
  ...
  const [form, setForm] = useState({ name:"", description:"", price:"", category:"Meals", imageUrl:"" });
  ...
  const fetchItems = async () => {
    try { const r = await API.get(MENU_URL); setItems(r.data); }
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editItem) await API.put(`${MENU_URL}/${editItem._id}`, form);
      else await API.post(MENU_URL, form);
  ...
  const handleDelete = async (id) => {
    ...
    await API.delete(`${MENU_URL}/${id}`); fetchItems();
  };
  ...
  const updateQty = (id, qty) => {
    if (qty < 1) { setCart(prev => prev.filter(c => c._id !== id)); return; }
  ...
  const placeOrder = async () => {
    ...
    try {
      for (const item of cart) {
        await API.post(ORDER_URL, {
          customerName: customerName.trim(),
          items: [{ menuItem: item._id, quantity: item.qty }],
          totalPrice: item.price * item.qty,
          paymentMethod: payMethod,
          paymentStatus: payMethod === "Cash" ? "Pending" : "Paid",
        });
      }
      setOrderSuccess(`Order placed for ${customerName}! Total: ₱${cartTotal}`);
      setCart([]); setCustomerName(""); setPayMethod("Cash");
  ...
  const PAYMENT_OPTIONS = [
    { id:"GCash",   icon:"gcash",   color:"#0075C9" },
    { id:"PayMaya", icon:"paymaya", color:"#00A862" },
    { id:"PayPal",  icon:"paypal",  color:"#003087" },
    { id:"Cash",    icon:"cash",    color:"#C9922A" },
  ];

  return (
    <div>
      <div className="page-header">
        <p className="page-eyebrow">Management Panel</p>
        <h1 className="page-title">Menu & Orders</h1>
      </div>

      {/* Tabs */}
      <div className="customer-tabs" style={{ marginBottom:"2rem" }}>
        <button className={activeTab === "menu" ? "ctab-active" : "ctab"} onClick={() => setActiveTab("menu")}>🍽 Menu Management</button>
        <button className={activeTab === "pos"  ? "ctab-active" : "ctab"} onClick={() => setActiveTab("pos")}>🛒 Place Order (POS)</button>
      </div>
      ...
      {activeTab === "menu" && (
        <div>
          <div className="form-card">
            ...
            <div className="form-grid">
              ...
              <div className="form-group full">
                ...
                <input value={form.imageUrl} onChange={e => setForm({...form, imageUrl:e.target.value})} placeholder="Right-click any food photo online → Copy image address"/>
                {form.imageUrl && (
            ...
            <div className="form-actions">
              <button className="btn-primary" onClick={handleSubmit}>{editItem ? "Update Dish" : "Add to Menu"}</button>
              {editItem && (
                <button className="btn-secondary" onClick={() => { setEditItem(null); setForm({ name:"", description:"", price:"", category:"Meals", imageUrl:"" }); }}>Cancel</button>
              )}
            </div>
          </div>

          <div style={{ background:"rgba(232,160,32,0.06)", border:"1px solid var(--b1)", padding:"1rem 1.25rem", marginBottom:"2rem", fontSize:"0.78rem", color:"var(--t3)", lineHeight:1.6 }}>
            💡 <strong style={{ color:"var(--g3)" }}>How to add a dish photo:</strong> Go to Google Images → search dish name → right-click any photo → <em>"Copy image address"</em> → paste in Image URL above!
          </div>
          ...
          {loading ? <p className="loading">◆ Preparing the menu… ◆</p>
          : filtered.length === 0 ? (
      ...
      {activeTab === "pos" && (
        <div style={{ display:"grid", gridTemplateColumns:"1fr 380px", gap:"2rem", alignItems:"start" }}>
                  ...
                  <div className="card" key={item._id} style={{ cursor:"pointer" }} onClick={() => addToCart(item)}>
                    {item.imageUrl ? (
                      <div style={{ position:"relative" }}>
                        <img src={item.imageUrl} alt={item.name}
                          style={{ width:"100%", height:130, objectFit:"cover", border:"1px solid var(--b1)", display:"block" }}
                          onError={e => e.target.style.display="none"}/>
                        <span style={{ position:"absolute", top:6, right:6, background:"rgba(0,0,0,0.75)", color:"var(--g1)", fontFamily:"Playfair Display,serif", fontSize:"0.9rem", padding:"3px 8px", border:"1px solid var(--b2)" }}>₱{item.price}</span>
                      </div>
                    ) : (
                      <div className="card-top">
                        <div className="card-icon"><span>{getFoodIcon(item.name, item.category)}</span></div>
                    ...
                    <button className="btn-primary" style={{ marginTop:4 }}>+ Add</button>
                  </div>
                ))}
              </div>
            )}
          ...
          {/* Order sidebar */}
          <div className="card" style={{ position:"sticky", top:20, background:"rgba(10,5,0,0.92)", backdropFilter:"blur(16px)" }}>
            <div style={{ textAlign:"center", borderBottom:"1px solid var(--b2)", paddingBottom:12, marginBottom:16 }}>
              <h2 style={{ fontFamily:"Cinzel,serif", color:"var(--g1)", fontSize:"1.3rem", letterSpacing:"0.12em" }}>🛒 New Order</h2>
              <p style={{ fontSize:"0.62rem", color:"var(--g4)", letterSpacing:"0.3em", marginTop:4, fontFamily:"Cinzel,serif" }}>POINT OF SALE</p>
            </div>

            {/* Customer name */}
            <div className="form-group" style={{ marginBottom:14 }}>
              <label>Customer Name</label>
              <input value={customerName} onChange={e => setCustomerName(e.target.value)} placeholder="e.g. Juan Dela Cruz"/>
            </div>

            {/* Cart items */}
            {cart.length === 0 ? (
              <p style={{ color:"var(--t3)", textAlign:"center", padding:"20px 0", fontSize:"0.85rem", fontStyle:"italic" }}>Tap any dish to add it here</p>
            ) : (
              <div style={{ display:"flex", flexDirection:"column", gap:10, maxHeight:260, overflowY:"auto", marginBottom:14 }}>
                {cart.map(item => (
                  <div key={item._id} style={{ display:"flex", alignItems:"center", gap:10, padding:"8px 0", borderBottom:"1px solid var(--b1)" }}>
                    <span style={{ fontSize:"1.2rem" }}>{getFoodIcon(item.name, item.category)}</span>
                    <div style={{ flex:1 }}>
                      <p style={{ fontSize:"0.85rem", color:"var(--t1)", fontFamily:"Playfair Display,serif" }}>{item.name}</p>
                      <p style={{ fontSize:"0.72rem", color:"var(--t3)" }}>₱{item.price} each</p>
                    </div>
                    <div style={{ display:"flex", alignItems:"center", gap:8, border:"1px solid var(--b2)", padding:"2px 8px" }}>
                      <button className="qty-btn" onClick={() => updateQty(item._id, item.qty - 1)}>−</button>
                      <span style={{ fontSize:"0.9rem", color:"var(--t1)", minWidth:16, textAlign:"center" }}>{item.qty}</span>
                      <button className="qty-btn" onClick={() => updateQty(item._id, item.qty + 1)}>+</button>
                    </div>
                    <span style={{ fontSize:"0.9rem", color:"var(--g2)", minWidth:50, textAlign:"right", fontFamily:"Playfair Display,serif" }}>₱{item.price * item.qty}</span>
                  </div>
                ))}
              </div>
            )}
            ...
            {/* Total */}
            {cart.length > 0 && (
              <div style={{ display:"flex", justifyContent:"space-between", padding:"12px 0", borderTop:"1px solid var(--b2)", marginBottom:14 }}>
                <span style={{ fontFamily:"Cinzel,serif", fontSize:"0.72rem", letterSpacing:"0.2em", color:"var(--g3)", textTransform:"uppercase" }}>◆ Total</span>
                <span style={{ fontFamily:"Playfair Display,serif", fontSize:"1.5rem", background:"linear-gradient(135deg,var(--g3),var(--g1))", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent", backgroundClip:"text" }}>₱{cartTotal}</span>
              </div>
            )}

            {/* Payment method */}
            <div style={{ marginBottom:14 }}>
              <p style={{ fontSize:"0.58rem", fontFamily:"Cinzel,serif", letterSpacing:"0.22em", textTransform:"uppercase", color:"var(--g4)", marginBottom:8 }}>Payment Method</p>
                ...
                {PAYMENT_OPTIONS.map(p => (
                  <div key={p.id}
                    ...
                    style={{
                      ...
                      border: payMethod === p.id ? `1px solid ${p.color}` : "1px solid var(--b1)",
                      background: payMethod === p.id ? `${p.color}15` : "rgba(0,0,0,0.2)",
                      transition:"all 0.2s",
                    }}>
                    <img src={PAY_ICONS[p.icon]} alt={p.id} style={{ width:24, height:24, objectFit:"contain", borderRadius: p.id==="GCash"?5:3 }}/>
                    <span style={{ fontSize:"0.68rem", fontFamily:"Cinzel,serif", letterSpacing:"0.06em", color: payMethod === p.id ? p.color : "var(--t2)", fontWeight: payMethod === p.id ? 700 : 400 }}>{p.id}</span>
                    {payMethod === p.id && <span style={{ marginLeft:"auto", color:p.color, fontSize:"0.8rem" }}>✓</span>}
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Success message */}
            {orderSuccess && <p className="success-msg" style={{ marginBottom:10 }}>✓ {orderSuccess}</p>}

            {/* Place order button */}
            <button className="btn-primary btn-full" onClick={placeOrder} disabled={placing || cart.length === 0}>
              {placing ? "Placing Order…" : `Place Order — ₱${cartTotal}`}
            </button>

            {cart.length > 0 && (
              <button onClick={() => setCart([])} style={{ width:"100%", marginTop:8, background:"transparent", border:"1px solid var(--b1)", color:"var(--t3)", padding:"8px", fontSize:"0.65rem", fontFamily:"Cinzel,serif", letterSpacing:"0.12em", textTransform:"uppercase", cursor:"pointer" }}>
                Clear Cart
              </button>
            )}
          </div>
